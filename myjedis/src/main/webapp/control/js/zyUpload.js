	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm"  action="/mj/fileUpload" enctype="multipart/form-data" method="post">';
	            	...
	            	html += '						<input id="file" type="file" name="hhh">';
	            	html += '					</div>';
	            	html += '				</div>';
					...
					html += '			<input type="submit" id="fileSubmit" class="upload_submit_btn" value="确认上传文件"></input>';
					html += '		</div>';
		            ...
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="file" type="file" size="30" name="fileselect[]" '+multiple+'>';
